...
class CustomVectorizer(CountVectorizer): 
    ...
    def build_analyzer(self):
        ...
        def analyser(doc):
            ...
            lemmatized_tokens = [lemmatizer.lemmatize(t) for t in list_of_tokens]
...
def build_vectorizer_model():
    ...
    stopwords_list = nltk_stopwords + course_prefixes + ge_areas + university_specific_words + other_words
    vectorizer = CustomVectorizer(ngram_range=(1, 1), stop_words=stopwords_list)
    return vectorizer

def build_topic_model():
    umap_model = UMAP(n_neighbors=75, random_state=RANDOM_STATE) # other good n_neighbors options: 60 | 80
...
if __name__ == "__main__":
    ...
    NUM_TOPICS_OPTIONS = [30]
    TOP_N_WORDS_OPTIONS = [20]
    ...
    for top_n_words in TOP_N_WORDS_OPTIONS:
        for num_topics in NUM_TOPICS_OPTIONS:
            ...
            topic_model.update_topics(doc_corpus, vectorizer_model=vectorizer_model, top_n_words=top_n_words)
            topic_model.reduce_topics(doc_corpus, nr_topics=num_topics)

            topics = topic_model.get_topics()
            write_topics(topics, f'topic-models/rs-{RANDOM_STATE}/{num_topics}topics-{top_n_words}words.json')
